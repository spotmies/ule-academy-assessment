    ...
    return (
        <div className="bg-white rounded-2xl p-4 shadow-[0_8px_20px_rgba(15,23,42,0.12)] border border-gray-200 mb-3 print:hidden">
            ...
            {questions.map((item) => (
                <div
                    key={item.qId}
                    className="mb-2.5 p-2 rounded-[10px] bg-[#f9fafb] border border-gray-200"
                >
                    <div className="flex flex-wrap items-center justify-between gap-1.5 text-[12px]">
                        ...
                        <div className="flex gap-1 flex-nowrap">
                            {[1, 2, 3, 4, 5].map((val) => (
                                <label
                                    key={val}
                                    className="text-[11px] px-1 py-0.5 rounded-full border border-gray-300 cursor-pointer inline-flex items-center gap-0.5 bg-white select-none hover:bg-gray-50"
                                >
                                    <input
                                        type="radio"
                                        name={`olq_q_${item.qId}`}
                                        value={val}
                                        checked={answers[item.qId] === val}
                                        onChange={() => onChange(item.qId, val)}
                                        className="m-0"
                                    />
                                    {val}
                                </label>
